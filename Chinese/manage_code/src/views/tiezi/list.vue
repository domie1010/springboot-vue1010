	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.biaoti" placeholder="标题"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('tiezi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('tiezi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('tiezi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('tiezi','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('tiezi','各内容分类数据统计')">各内容分类数据统计</el-button>
					<el-button type="warning" @click="echartClick2" v-if="btnAuth('tiezi','日发帖量数据统计')">日发帖量数据统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="审核" v-if="btnAuth('tiezi','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('tiezi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
						<el-button v-if="btnAuth('tiezi','举报')" type="success" @click="jubaoxinxiCrossAddOrUpdateHandler(scope.row,'cross','是','','')">举报</el-button>
						<el-button v-if="btnAuth('tiezi','查看评论')" type="warning" @click="commentClick(scope.row.id)">查看评论</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'各内容分类数据统计'" width="70%">
			<el-tabs v-model="echartActive" class="demo-tabs" @tab-change="echartTabClick" type="card">
			    <el-tab-pane label="各内容分类数据统计" name="1"></el-tab-pane>
			    <el-tab-pane label="日发帖量数据统计" name="2"></el-tab-pane>
			</el-tabs>
			<div v-if="echartActive==1" id="neirongfenleiEchart1" style="width:100%;height:600px;"></div>

			<div v-if="echartActive==2" id="fabushijianEchart2" style="width:100%;height:600px;"></div>
		...
		<jubaoxinxiFormModel ref="jubaoxinxiFormModelRef" @formModelChange="formModelChange"></jubaoxinxiFormModel>
	</div>
</template>
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.biaoti&&searchQuery.value.biaoti!=''){
			params['biaoti'] = '%' + searchQuery.value.biaoti + '%'
		}
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var neirongfenleiEchart1 = echarts.init(document.getElementById("neirongfenleiEchart1"),'macarons');
			context?.$http({
				url: `tiezi/group/neirongfenlei`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				neirongfenleiEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    neirongfenleiEchart1.resize();
				};
			})
		})
	}
	...
	const echartClick2 = ()=>{
		...
		echartVisible.value = true
		nextTick(()=>{
			var fabushijianEchart2 = echarts.init(document.getElementById("fabushijianEchart2"),'macarons');
			context?.$http({
				url: `tiezi/group/fabushijian`,
				method: 'get'
			}).then(res=>{
				let obj = res.data.data
				let xAxis = [];
				...
				let pArray = []
				for(let i=0;i<obj.length;i++){
				    xAxis.push(obj[i].fabushijian);
				    yAxis.push(parseFloat((obj[i].total)));
				    pArray.push({
				        value: parseFloat((obj[i].total)),
				        name: obj[i].fabushijian
				    })
				}
				var option = {};
                option = {
				...
				// 使用刚指定的配置项和数据显示图表。
				fabushijianEchart2.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    fabushijianEchart2.resize();
				};
			})
		})
	}
	...
	const approvalClick = (row) => {
		let params = {
			...
			neirongfenlei: row.neirongfenlei,
			neirongxiangqing: row.neirongxiangqing,
	...
	const commentClick=(id)=>{
		context?.$router.push('/discusstiezi?refid=' + id)
	}
	import jubaoxinxiFormModel from '@/views/jubaoxinxi/formModel'
	const jubaoxinxiFormModelRef = ref(null)
    const jubaoxinxiCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			jubaoxinxiFormModelRef.value.init(row.id,'cross','举报',row,'tiezi',statusColumnName,tips,statusColumnValue)
		})
    }
	//查询审核状态列表
