	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('jubaoxinxi','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('jubaoxinxi','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('jubaoxinxi','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('jubaoxinxi','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column
					 ...
					 align="left" 
					 header-align="left"
					 prop="jubaorenzhanghao"
					label="举报人账号">
					<template #default="scope">
						{{scope.row.jubaorenzhanghao}}
					</template>
				</el-table-column>
				...
				<el-table-column label="审核" v-if="btnAuth('jubaoxinxi','审核')" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('jubaoxinxi','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const approvalClick = (row) => {
		let params = {
			...
			jubaoyuanyin: row.jubaoyuanyin,
			jubaoshijian: row.jubaoshijian,
			jubaorenzhanghao: row.jubaorenzhanghao,
			shhf: row.shhf,
			sfsh: row.sfsh,
		}
		nextTick(() => {
